...
OfferExchange::OfferExchange(AccountManager& accountManager, LedgerDelta& delta, LedgerManager& ledgerManager, AssetPairFrame::pointer assetPair, BalanceFrame::pointer commissionBalance)
	: mDelta(delta), mLedgerManager(ledgerManager), mAccountManager(accountManager), mAssetPair(assetPair), mCommissionBalance(commissionBalance)
{
...
bool OfferExchange::getQuoteAmountBasedOnFee(OfferEntry const& offer, int64_t& quoteAmountBasedOnFee)
{
	...
	return bigDivide(quoteAmountBasedOnFee, offer.fee, 100 * ONE, percentFee, ROUND_DOWN);
}

int64 OfferExchange::getMatchPrice(OfferEntry const& buy, OfferEntry const& sell)
{
...
ExchangeResult OfferExchange::exchange(OfferEntry& offerA, OfferEntry& offerB)
{
	...
	int64_t buyingOfferQuoteAmount = std::min(quoteAmountBasedOnLockedFee, buyingOffer.quoteAmount);

	auto match = exchange(buyingOffer.baseAmount, buyingOfferQuoteAmount, sellingOffer.baseAmount, sellingOffer.quoteAmount, matchPrice);
	if (!setFeeToPay(match.buyerFee, match.quoteDelta, buyingOffer.percentFee) || !setFeeToPay(match.sellerFee, match.quoteDelta, sellingOffer.percentFee))
	{
		ExchangeResult result{};
		result.type = ExchangeResultType::RESULT_OVERFLOW;
		return result;
	...
	{
		LOG(ERROR) << "baseAmountA: " << offerA.baseAmount << " quoteAmountA: " << offerA.quoteAmount << " baseAmountB: " << offerB.baseAmount << " quoteAmountB:" << offerB.quoteAmount << " matchingPrice: " << offerB.price << " offerB is buy:" << offerB.isBuy << " offerBPercentFee: " << offerB.percentFee << " offerAPercentFee: " << offerA.percentFee;
...
bool OfferExchange::setFeeToPay(int64_t& feeToPay, int64_t quoteAmount, int64_t percentFee)
{
...
ExchangeResult
OfferExchange::exchange(int64_t buyerBase, int64_t buyerQuote, int64_t sellerBase, int64_t sellerQuote, int64_t matchPrice)
{
	ExchangeResult result{};
	...
	if (!bigDivide(result.quoteDelta, result.baseDelta, matchPrice, ONE, ROUND_UP))
	{
		result.type = ExchangeResultType::RESULT_OVERFLOW;
		return result;
	}
	...
	if (!bigDivide(result.baseDelta, result.quoteDelta, ONE, matchPrice, ROUND_DOWN))
	{
		result.type = ExchangeResultType::RESULT_OVERFLOW;
		return result;
	}
	...
	if (!bigDivide(quoteDeltaCandiate, result.baseDelta, matchPrice, ONE, ROUND_UP))
	{
		result.type = ExchangeResultType::RESULT_OVERFLOW;
		return result;
	}
...
void OfferExchange::unlockBalancesForTakenOffer(OfferFrame& offer, BalanceFrame::pointer baseBalance, BalanceFrame::pointer quoteBalance)
{
	BalanceFrame::Result result;
	if (offer.getOffer().isBuy)
	{
		int64_t lockedAmount = offer.getOffer().quoteAmount + offer.getOffer().fee;
...
bool OfferExchange::isOfferPriceMeetAssetPairRestrictions(AssetPairFrame::pointer assetPair, int64_t offerPrice)
{
	int64_t minAllowedPrice = assetPair->getMinAllowedPrice();
	return offerPrice >= minAllowedPrice;
}

void OfferExchange::offerMatched(OfferEntry& offer, BalanceFrame::pointer baseBalance, BalanceFrame::pointer quoteBalance, ExchangeResult match)
{
	...
	if (offer.isBuy)
	{
		...
		isBalanceValid = baseBalance->addBalance(match.baseDelta) && quoteBalance->addLocked(-amountToCharge);
	...
	else 
	{
		...
		isBalanceValid = baseBalance->addLocked(-match.baseDelta) && quoteBalance->addBalance(amountToAdd);
	}

	if (!isBalanceValid)
		throw std::runtime_error("Failed to handel offer match - unable to unlock balace");

	if (offer.baseAmount < 0 || offer.quoteAmount < 0 || offer.fee < 0)
		throw std::runtime_error("Failed to handel offer match - unable to update base or quote amount for offer");
}

ClaimOfferAtom OfferExchange::createOfferClaim(OfferEntry const& offerB, ExchangeResult const& match)
{
...
OfferExchange::crossOffer(OfferEntry& offerA, BalanceFrame::pointer baseBalanceA, BalanceFrame::pointer quoteBalanceA, OfferFrame& offerFrameB)
{
	...
	isOfferValid = isOfferValid && isOfferPriceMeetAssetPairRestrictions(mAssetPair, offerFrameB.getPrice());
	...
	{
		LOG(ERROR) << "baseAmountA: " << offerA.baseAmount << " quoteAmountA: " << offerA.quoteAmount << " baseAmountB: " << offerB.baseAmount << " quoteAmountB:" << offerB.quoteAmount << " matchingPrice: " << offerFrameB.getPrice() << " offerB is buy:" << offerB.isBuy;
	...
	mFeePaidByA += (offerA.isBuy ? exchangeResult.buyerFee : exchangeResult.sellerFee);
...
bool OfferExchange::offerNeedsMore(OfferEntry& offer)
{
	...
	if (offer.isBuy)
	{
		...
		int64_t availableQuoteAmount = std::min(quoteAmountBasedOnFee, offer.quoteAmount);
		int64_t canBuy = 0;
		if (!bigDivide(canBuy, availableQuoteAmount, ONE, offer.price, ROUND_DOWN) || canBuy == 0)
			return false;
...
OfferExchange::convertWithOffers(
	OfferEntry& offerA, BalanceFrame::pointer baseBalanceA, BalanceFrame::pointer quoteBalanceA,
    std::function<OfferFilterResult(OfferFrame const&)> filter)
{
    ...
    while (offerNeedsMore(offerA))
    {
        ...
        for (auto& offerB : retList)
        {
			...
			CrossOfferResult cor = crossOffer(offerA, baseBalanceA, quoteBalanceA, *offerB);
