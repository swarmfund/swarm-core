...
SignatureValidator::SignatureValidator(Hash contentHash, xdr::xvector<DecoratedSignature, 20> signatures)
{
...
bool SignatureValidator::isAccountTypeAllowed(AccountFrame& account, vector<AccountType> allowedAccountTypes)
{
...
SignatureValidator::Result SignatureValidator::check(std::vector<PublicKey> keys, int signaturesRequired)
{
...
SignatureValidator::Result SignatureValidator::check(Application& app, Database& db, AccountFrame& account, SourceDetails& sourceDetails)
{
	if ((account.getBlockReasons() | sourceDetails.mAllowedBlockedReasons) != sourceDetails.mAllowedBlockedReasons)
