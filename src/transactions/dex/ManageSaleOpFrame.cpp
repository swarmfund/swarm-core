...
ManageSaleOpFrame::ManageSaleOpFrame(Operation const& op,
    ...
    , mManageSaleOp(mOperation.body.manageSaleOp())
{
}
...
bool
ManageSaleOpFrame::amendUpdateSaleDetailsRequest(Database& db,
                                                 LedgerDelta& delta)
{
...
bool
ManageSaleOpFrame::amendUpdateEndTimeRequest(Database& db, LedgerDelta& delta)
{
    auto requestFrame = ReviewableRequestHelper::Instance()->loadRequest(
...
bool
ManageSaleOpFrame::createPromotionUpdateRequest(LedgerDelta& delta,
                                                LedgerManager& ledgerManager,
                                                Database& db,
                                                SaleState saleState,
                                                AccountID const& masterID)
{
    ...
    auto requestFrame = ReviewableRequestFrame::createNew(
        delta, getSourceID(), masterID, referencePtr,
        ledgerManager.getCloseTime());
...
bool
ManageSaleOpFrame::amendPromotionUpdateRequest(Database& db, LedgerDelta& delta)
{
    auto requestFrame = ReviewableRequestHelper::Instance()->loadRequest(
                           ...
                           LedgerManager& ledgerManager)
{
    ...
    case ManageSaleAction::CREATE_PROMOTION_UPDATE_REQUEST:
    {
        ...
        return createPromotionUpdateRequest(
            delta, ledgerManager, db, saleFrame->getState(), app.getMasterID());
    }
    default:
        CLOG(ERROR, Logging::OPERATION_LOGGER)
