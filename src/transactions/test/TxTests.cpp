    ...
    else
    {
        ...
        if (!toAccount) {
			...
			if (referrer)
			{
				...
				auto referrerFrame = accountHelper->loadAccount(referrerAccountID, app.getDatabase());
				if (referrerFrame && !(referrerFrame->getID() == app.getMasterID()))
				{
					auto feeShare = feeHelper->loadForAccount(FeeType::REFERRAL_FEE, app.getBaseAsset(), 0, referrerFrame, 0, app.getDatabase());
