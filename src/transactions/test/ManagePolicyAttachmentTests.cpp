...
TEST_CASE("Manage policy attachment", "[tx][manage_policy_attachment]") {
    ...
    {
        auto actorWithAccID = managePAHelper.createActorForAccountID(account.key.getPublicKey());
        // not-existent identity policy
        auto creationOpInput = managePAHelper.createCreationOpInput(2, actorWithAccID);
    ...
    SECTION("Successful creation") {
        {
            ...
            auto actorWithAccID = managePAHelper.createActorForAccountID(account.key.getPublicKey());
            auto creationOpInput = managePAHelper.createCreationOpInput(1, actorWithAccID);

            managePAHelper.applyManagePolicyAttachment(account, creationOpInput);
        }
        {
            // create for account type
            auto actorWithAccType = managePAHelper.createActorForAccountType(AccountType::GENERAL);
            auto creationOpInput = managePAHelper.createCreationOpInput(1, actorWithAccType);
        ...
        }
        {
            ...
            auto creationOpInput = managePAHelper.createCreationOpInput(1, actorAny);
    ...
    {
        auto actorWithAccID = managePAHelper.createActorForAccountID(SecretKey::random().getPublicKey());
        auto creationOpInput = managePAHelper.createCreationOpInput(1, actorWithAccID);
    ...
    {
        // create first policy attachment with identity policy created before section
        auto actorWithAccType = managePAHelper.createActorForAccountType(AccountType::GENERAL);
        {
            auto creationOpInput = managePAHelper.createCreationOpInput(1, actorWithAccType);
            managePAHelper.applyManagePolicyAttachment(account, creationOpInput);
        ...
        for (; i <= 100; ++i) {
            ...
            auto creationOpInput = managePAHelper.createCreationOpInput(i, actorWithAccType);
        ...
        managePAHelper.applyManagePolicyAttachment(account,
                                                   creationOpInput,
                                                   ManagePolicyAttachmentResultCode::POLICY_ATTACHMENTS_LIMIT_EXCEEDED);
