...
namespace stellar
{
	...
	void ExternalSystemAccountIDHelper::storeUpdateHelper(LedgerDelta& delta, Database& db, const bool insert, LedgerEntry const& entry)
	{
		...
		const auto version = static_cast<int32_t>(externalSystemAccountIDEntry.ext.v());
	...
	ExternalSystemAccountIDHelper::getLedgerKey(LedgerEntry const& from)
	{
		...
		ledgerKey.externalSystemAccountID().accountID = from.data.externalSystemAccountID().accountID;
		ledgerKey.externalSystemAccountID().externalSystemType = from.data.externalSystemAccountID().externalSystemType;
		return ledgerKey;
	}
