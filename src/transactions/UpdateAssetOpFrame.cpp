...
ReviewableRequestFrame::pointer UpdateAssetOpFrame::getUpdatedOrCreateReviewableRequest(Database & db, LedgerDelta & delta)
{
	ReviewableRequestFrame::pointer request = getOrCreateReviewableRequest(db, delta, ReviewableRequestType::ASSET_UPDATE);
	if (!request)
		return nullptr;
...
{
	if (!isAssetValid(mAssetUpdateRequest.code)) {
