...
namespace stellar {
    ...
    void SaleFrame::subCurrentCap(AssetCode const &asset, uint64_t const amount) {
        ...
        if (saleQuoteAsset.currentCap < amount) {
            CLOG(ERROR, Logging::ENTRY_LOGGER)
                    << "Unexpected state: tring to substract from current cap amount exceeding it: "
                    << xdr::xdr_to_string(mSale) << " amount: " << amount;
    ...
    SaleFrame::pointer
    SaleFrame::createNew(uint64_t const &id, AccountID const &ownerID, SaleCreationRequest const &request,
        ...
        try {
            ...
            if (request.ext.v() == LedgerVersion::ALLOW_TO_MANAGE_SALE) {
                sale.ext.v(LedgerVersion::ALLOW_TO_MANAGE_SALE);
                const auto saleType = request.ext.saleTypeExt().typedSale.saleType();
                sale.ext.saleStates().saleTypeExt.typedSale.saleType(saleType);
            }
    ...
    uint64_t SaleFrame::getBaseAmountForCurrentCap(AssetCode const &asset) {
        ...
        if (!convertToBaseAmount(quoteAsset.price, quoteAsset.currentCap, baseAmount)) {
            CLOG(ERROR, Logging::ENTRY_LOGGER) << "Unexpected state: failed to conver to base amount current cap: "
                                               << xdr::xdr_to_string(mSale);
    ...
    SaleType SaleFrame::getSaleType(SaleEntry const &sale) {
        ...
        switch (version) {
            ...
            case LedgerVersion::ALLOW_TO_MANAGE_SALE:
                if (sale.ext.saleStates().saleTypeExt.typedSale.saleType() == DEFAULT_SALE_TYPE) {
                    return DEFAULT_SALE_TYPE;
                }
                return sale.ext.saleTypeExt().typedSale.saleType();
            default:
                CLOG(ERROR, Logging::ENTRY_LOGGER) << "Unexpected version of sale entry: "
                                                   << xdr::xdr_to_string(version);
    ...
    void SaleFrame::setSaleType(SaleEntry &sale, const SaleType saleType) {
        ...
        switch (version) {
            ...
            case LedgerVersion::ALLOW_TO_MANAGE_SALE:
                ...
                return;
            default:
                CLOG(ERROR, Logging::ENTRY_LOGGER) << "Unexpected ledger version of sale. version: "
                                                   << xdr::xdr_to_string(version);
